...
public class GoRangeIterationOnIllegalTypeInspection extends GoInspectionBase {
  ...
  protected GoVisitor buildGoVisitor(@NotNull final ProblemsHolder holder, @NotNull LocalInspectionToolSession session) {
    return new GoVisitor() {
      ...
      public void visitRangeClause(@NotNull GoRangeClause o) {
        ...
        GoExpression expression = o.getExpressionList().get(0);
  ...
  private static boolean isValidTypeRefExpression(GoType goType) {
    ...
    GoTypeReferenceExpression refExpr = goType.getTypeReferenceExpression();
    if (refExpr == null) return isValidType(goType);

    GoType baseType = GoPsiImplUtil.findBaseTypeFromRef(refExpr);
    if (!(baseType instanceof GoSpecType)) return false;

    goType = ((GoSpecType)baseType).getType();
    return isValidType(goType);
  }
}
